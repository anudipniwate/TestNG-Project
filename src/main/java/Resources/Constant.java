package Resources;

public class Constant {
	
	public static String username="Anudip";

	public static String password="Anudip123";
	
	public static String expectedString="Please check your username and password. If you still can't log in, contact your Salesforce administrator.";
	
	public static String firstname="Anudip";
	
	public static String lastname="Niwate";
	
	public static String workmail="anudip@gmail.com";
	
	public static String conpanyname="Infosys";
	
	public static String userphone="9123456789";
}
